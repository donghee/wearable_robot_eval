...
def generate_launch_description():
    ...
    spawn_upper_limb_world = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_wearable_robot_description, 'launch', 'upper_limb.launch.py'),
        )
    ) 

    evaluate_upper_limb = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_wearable_robot_description, 'launch', 'evaluate_upper_limb.launch.py'),
        )
    ) 
    ...
    return LaunchDescription([
        start_world,
        spawn_human_world,
        TimerAction(period=0.1, actions = [spawn_upper_limb_world]),
        TimerAction(period=3.0, actions = [evaluate_upper_limb]),
    ])
